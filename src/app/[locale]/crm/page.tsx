...
export default async function CrmDashboard({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <CrmShell user={user} locale={locale}><div className="crm-page">
    ...
    {unattended.length > 0 && <section className="crm-section"><div className="crm-section-heading"><h2>Клиенты без следующего шага</h2><Link href={`/${locale}/crm/clients?attention=no_next_task`}>Открыть список</Link></div><div className="crm-inline-list">{unattended.slice(0, 6).map((client) => {
      const clientTitle = (client.companyName && client.companyName.trim() !== '-')
        ? client.companyName
        : (client.contactName?.trim() || client.suggestedService || client.sourcePlatform || 'Лид');
      return <Link key={client.id} href={`/${locale}/crm/clients/${client.id}`}>{clientTitle}<span>{statusLabels[client.status] || client.status}</span></Link>;
    })}</div></section>}
