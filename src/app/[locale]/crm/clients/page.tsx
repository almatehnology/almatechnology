...
import { listActiveUsers, listClients, type ClientFilters } from '@/lib/crm';
import { formatDateTime, statusLabels } from '@/lib/crm-format';
import { requireUser } from '@/lib/session';

type SearchParams = { scope?: 'mine' | 'all' | 'pool' | 'archived'; status?: string; ownerId?: string; search?: string; attention?: 'overdue' | 'no_next_task' };
...
export default async function ClientsPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<SearchParams> }) {
  ...
  const selected: ClientFilters = { scope: filters.scope || 'mine', status: filters.status, ownerId: filters.ownerId, search: filters.search, attention: filters.attention };
  ...
  return <CrmShell user={user} locale={locale}><div className="crm-page">
    ...
    <form className="crm-filter-bar" action={base}>
      ...
      <label className="crm-search"><Search size={17} /><input name="search" defaultValue={selected.search} placeholder="Компания, человек, email, телефон" /></label>
