...
export default async function TeamPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <CrmShell user={user} locale={locale}><div className="crm-page"><header className="crm-page-header"><div><p className="crm-eyebrow">АДМИНИСТРИРОВАНИЕ</p><h1>Команда</h1><p>Назначайте каждому сотруднику любые этапы конвейера. Администратор имеет доступ ко всем действиям независимо от выбранных ролей.</p></div></header><div className="crm-team-grid"><CreateUserForm locale={locale} /><section className="crm-card"><h2>Пользователи</h2><div className="crm-users-list">{users.map((member) => <article key={member.id} className={member.banned ? 'disabled' : ''}><div><strong>{member.name}</strong><span>{member.username || member.email}</span><small>{member.role === 'admin' ? 'Администратор' : 'Сотрудник'} · {member.email}</small><div className="crm-role-badges">{member.salesRoles.length ? member.salesRoles.map((role) => <span key={role}>{salesRoleLabels[role]}</span>) : <span>Рабочие роли не назначены</span>}</div></div>{member.id === user.id ? <span className="crm-self">Вы</span> : <UserControls locale={locale} id={member.id} banned={Boolean(member.banned)} salesRoles={member.salesRoles} />}</article>)}</div></section></div></div></CrmShell>;
}
